...
//TODO REFACTOR this to read buffer then extract message
  async getMessage() {
      const ftStatusResult = await this.getStatus()
      let rxBuffer = Buffer.alloc(0)
      let isUpdate = false
      let position = 0
      //console.log( 'get status result is ' + ftStatusResult.ftStatus + 'rx queue is' + ftStatusResult.rxQueue)
      if ( ftStatusResult.ftStatus === FT_STATUS.FT_OK && ftStatusResult.rxQueue > 0)
      {
        ...
        const ftStatus = await this.read(rxBuffer)
